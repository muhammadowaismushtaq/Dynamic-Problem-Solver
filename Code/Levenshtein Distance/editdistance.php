...
<html>
<head>
	<title>Lavenshtein Distance</title>
...
<body>
	...
	?>
<nav>
 	<h2> LAVENSHTEIN DISTANCE (EDIT DISTANCE) PROBLEM </h2>
...
<div id="working">
<h3 style="color:black;">EDIT DISTANCE ALGORITHM</h3>
<div id="code">
	<span>for(let i=1;i<=string1.length;i++)</span>{<br>
		<span>&emspfor(let j=1;j<=string2.length;j++)</span>{<br>
			<span>&emsp;&emsp;&emsp;if(string1[i-1] == string2[j-1])</span>{<br>
				<span>&emsp;&emsp;&emsp;EditDistance_table[i][j] = EditDistance_table[i-1][j-1];</span>
			<br>
			<span>}&emsp;&emsp;else{</span><br>
				<span>&emsp;&emsp;&emsp;EditDistance_table[i][j] = 1+Math.min(EditDistance_table[i-1][j],EditDistance_table[i][j-1],EditDistance_table[i-1][j-1]);</span><br>
			&emsp;&emsp;}<br>
		&emsp;}<br>
...
<script type="text/javascript" >
	...
	var EditDistance_table=[];
		for(let i=0;i<=string1.length;i++)
		{
		EditDistance_table[i]=[];
		material[i]=[];
		for(let j=-2;j<=string2.length;j++)
		{
			...
			if(i==0)
			{
				EditDistance_table[i][j]=j;
				material[i][j]='i = '+i+'   j = '+j+'<br>EditDistance_table[i][j]=j;';
				//material[i][j]='EditDistance_table[i][0]=0;';
				// $("#steps").append('<span class="step'+(i) + (j)+'" style="color:#fff">i = '+i+'   j = '+j+'</span><br>');
				// $("#steps").append('<span class="step'+(i) + (j)+'" style="color:#fff">EditDistance_table[i][0]=0;</span><br>');
			}
			else if(j==0)
			{
				EditDistance_table[i][j]=i;
				material[i][j]='i = '+i+'   j = '+j+'<br>EditDistance_table[i][j]=i;';
			}
			}
			else
			{
	...
	for(let i=1;i<=string1.length;i++)
	{
		for(let j=1;j<=string2.length;j++)
		{
			...
			{
				//<br>EditDistance_table[i][j] = '+Math.min(EditDistance_table[i-1][j],1+EditDistance_table[i][j-number[i-1]])+';
				// <br>EditDistance_table[i][j] = Math.min('+EditDistance_table[i-1][j]+',1+'+EditDistance_table[i][j-number[i-1]+'])'
				// <br>EditDistance_table[i][j] = Math.min(EditDistance_table['+i-1+']['+j+'],1+EditDistance_table['+i+']['+j+'-number['+i-1+']])'
				EditDistance_table[i][j] = EditDistance_table[i-1][j-1];
				material[i][j]='i = '+i+'   j = '+j+'<br>EditDistance_table[i][j] = EditDistance_table['+i+'-1]['+j+'-1]+1 <br>EditDistance_table[i][j] = EditDistance_table['+(i-1)+']['+(j-1)+']+1<br>So,EditDistance_table[i][j] = '+(EditDistance_table[i-1][j-1]+1)+'';
				// $("#steps").append('<span id="step'+(i) + (j)+'" style="color:#fff"></span>');
				// $("#steps").append('<span id="step'+(i) + (j)+'" style="color:#fff"></span>');
				// $("#steps").append('<span id="step'+(i) + (j)+'" style="color:#fff">'+EditDistance_table[i][j]+'</span>');
			}
			else
			{
				EditDistance_table[i][j] = 1+Math.min(EditDistance_table[i-1][j],EditDistance_table[i][j-1],EditDistance_table[i-1][j-1]);
				material[i][j]='i = '+i+'   j = '+j+'<br>EditDistance_table[i][j]= 1+Math.min(EditDistance_table[i-1][j],EditDistance_table[i][j-1],EditDistance_table[i-1][j-1]);<br>EditDistance_table[i][j]= 1+Math.min(EditDistance_table['+i+'-1]['+j+'],EditDistance_table['+i+']['+j+'-1],EditDistance_table['+i+'-1]['+j+'-1]);<br>EditDistance_table[i][j]= 1+Math.min(EditDistance_table['+(i-1)+']['+j+'],EditDistance_table['+i+']['+(j-1)+'],EditDistance_table['+(i-1)+']['+(j-1)+']);<br>EditDistance_table[i][j]= 1+Math.min('+(EditDistance_table[i-1][j])+','+(EditDistance_table[i][j-1])+','+(EditDistance_table[i-1][j-1])+');<br>So, <br>EditDistance_table[i][j]= '+(1+Math.min(EditDistance_table[i-1][j],EditDistance_table[i][j-1],EditDistance_table[i-1][j-1]))+'';
	...
	for(let i=0;i<=string1.length;i++)
	{
		...
		for(let j=-2;j<=string2.length;j++)
		{
			...
			$("#content"+i).append('<td id="id'+(i) + (j)+'" style="color:#fff;">'+EditDistance_table[i][j]+'</td>');
