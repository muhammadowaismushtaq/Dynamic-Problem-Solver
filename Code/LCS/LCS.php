...
<div id="working">
<div id="code">
<h3 style="color:black;">LCS ALGORITHM</h3>
	<span>for(let i=1;i<=string1.length;i++){</span><br>
		<span>&emsp;for(let j=1;j<=string2.length;j++)</span>{<br>
			<span>&emsp;&emsp;if(string1[i-1] == string2[j-1])</span>{<br>
				<span>&emsp;&emsp;&emsp;LCS_table[i][j] = LCS_table[i-1][j-1] + 1;</span>}<br>
			<span>&emsp;&emsp;else</span>{<br>
				<span>&emsp;&emsp;&emsp;LCS_table[i][j] = Math.max(LCS_table[i-1][j],LCS_table[i][j-1]);</span><br>
			&emsp;&emsp;}<br>
		&emsp;}<br>
...
<script type="text/javascript" >
	...
	var LCS_table=[];
		...
		for(let j=-2;j<=string2.length;j++)
		{
			...
			{
				LCS_table[i][j]=0;
				material[i][j]='i = '+i+'   j = '+j+'<br>LCS_table[i][j]=0;';
				//material[i][j]='LCS_table[i][0]=0;';
				// $("#steps").append('<span class="step'+(i) + (j)+'" style="color:#fff">i = '+i+'   j = '+j+'</span><br>');
				// $("#steps").append('<span class="step'+(i) + (j)+'" style="color:#fff">LCS_table[i][0]=0;</span><br>');
			}
			}
			else
			{
	...
	for(let i=1;i<=string1.length;i++)
	{
		for(let j=1;j<=string2.length;j++)
		{
			...
			{
				//<br>LCS_table[i][j] = '+Math.min(LCS_table[i-1][j],1+LCS_table[i][j-number[i-1]])+';
				// <br>LCS_table[i][j] = Math.min('+LCS_table[i-1][j]+',1+'+LCS_table[i][j-number[i-1]+'])'
				// <br>LCS_table[i][j] = Math.min(LCS_table['+i-1+']['+j+'],1+LCS_table['+i+']['+j+'-number['+i-1+']])'
				LCS_table[i][j] = LCS_table[i-1][j-1] + 1;
				material[i][j]='i = '+i+'   j = '+j+'<br>LCS_table[i][j] = LCS_table['+i+'-1]['+j+'-1]+1 <br>LCS_table[i][j] = LCS_table['+(i-1)+']['+(j-1)+']+1<br>So,LCS_table[i][j] = '+(LCS_table[i-1][j-1]+1)+'';
				// $("#steps").append('<span id="step'+(i) + (j)+'" style="color:#fff"></span>');
				// $("#steps").append('<span id="step'+(i) + (j)+'" style="color:#fff"></span>');
				// $("#steps").append('<span id="step'+(i) + (j)+'" style="color:#fff">'+LCS_table[i][j]+'</span>');
			}
			else
			{
				LCS_table[i][j] = Math.max(LCS_table[i-1][j],LCS_table[i][j-1]);
				material[i][j]='i = '+i+'   j = '+j+'<br>LCS_table[i][j]= Math.max(LCS_table[i-1][j],LCS_table[i][j-1]);<br>LCS_table[i][j]= Math.max(LCS_table['+i+'-1]['+j+'],LCS_table['+i+']['+j+'-1]);<br>LCS_table[i][j]=LCS_table[i][j]= Math.max(LCS_table['+(i-1)+']['+j+'],LCS_table['+i+']['+(j-1)+']);<br>LCS_table[i][j]= Math.max('+(LCS_table[i-1][j])+','+(LCS_table[i][j-1])+');<br>So, <br>LCS_table[i][j]= '+Math.max(LCS_table[i-1][j],LCS_table[i][j-1])+'';
	...
	for(let i=0;i<=string1.length;i++)
	{
		...
		for(let j=-2;j<=string2.length;j++)
		{
			...
			$("#content"+i).append('<td id="id'+(i) + (j)+'" style="color:#fff;">'+LCS_table[i][j]+'</td>');
